...
Ast.findInnerMostNode = (ast, offset, predicate) => {
    function findInnermostNodeInternal(node) {
        if (node.getFullStart(ast) <= offset && node.end >= offset) {
            if (predicate(node)) {
                const innermostNode = ts.forEachChild(
                    node,
                    findInnermostNodeInternal
                );
...
Ast.findInnermostNodeOfAnyKind = (ast, offset) => {
    function findInnermostNodeOfAnyKindInternal(node) {
        if (node.getStart(ast) <= offset && node.end >= offset) {
            const innermostNode = ts.forEachChild(
                node,
                findInnermostNodeOfAnyKindInternal
            );
            return innermostNode || node;
        }
    }
...
Ast.visitVisibleSymbols = (node, callback) => {
    ...
    return (
        visitVisibleSymbolsInternal(node) ||
        visitVisibleSymbolsInternal(es5LibAst)
    );
};
...
Ast.findActiveTypeBinders = (
    ...
    if (leftSibling) {
        return Ast.findActiveTypeBindersInLeftSibling(
            leftSibling,
            symbol,
            startNode,
            stopNode
        ...
        return Ast.findActiveTypeBindersInParent(
            parent,
            symbol,
            startNode,
            stopNode
...
Ast.findActiveTypeBindersInLeftSibling = (
    node,
    symbol,
    startNode,
    stopNode
    ...
    return (
        Ast.findActiveTypeBindersInStatement(
            node,
            symbol,
            startNode,
            stopNode
...
Ast.findActiveTypeBindersInParent = (node, symbol, startNode, stopNode) => {
    ...
    if (
        ...
        return findActiveTypeBindersInCallExpression(
            node,
            symbol,
            startNode,
            stopNode
        ...
        return findActiveTypeBindersInCallSite(
            node,
            symbol,
            startNode,
            stopNode
        ...
        return findActiveTypeBindersInAssignmentLike(
            parent,
            symbol,
            startNode,
            stopNode
        ...
        return findActiveTypeBindersOutOfIfStatement(
            [parent.expression],
            symbol,
            startNode
        );
        ...
        return findActiveTypeBindersOutOfForStatement(
            parent,
            symbol,
            startNode
        );
...
Ast.findActiveTypeBindersInStatement = (node, symbol, startNode, stopNode) => {
    ...
    switch (node.kind) {
        ...
        case ts.SyntaxKind.SwitchStatement:
            return findActiveTypeBindersInSwitchStatement(
                node,
                symbol,
                startNode
            );
...
function findActiveTypeBindersInBlock(node, symbol, startNode) {
    if (!doesBlockAffectSymbol(node, symbol)) {
        return;
    }
    ...
    return Ast.findActiveTypeBindersInStatement(
        lastStatement,
        symbol,
        startNode,
        node
...
function findActiveTypeBindersInIfStatement(node, symbol, startNode) {
    ...
    for (const statement of statements) {
        const statementBinders = Ast.findActiveTypeBindersInStatement(
            statement,
            symbol,
            startNode
        );
    ...
    if (!hasElse(node)) {
        conditionsToSearch.add(
            statements[statements.length - 1].parent.expression
        );
    }
...
function findActiveTypeBindersOutOfIfStatement(conditions, symbol, startNode) {
    ...
    while (condition) {
        const conditionBinders = Ast.findActiveTypeBindersInLeftSibling(
            condition,
            symbol,
            startNode,
            condition.parent
    ...
    if (conditions.length) {
        ...
        binders.push(
            ...findActiveTypeBindersOutOfConditional(
                ifStatement,
                symbol,
                startNode
            )
...
function findActiveTypeBindersInSwitchStatement(node, symbol, startNode) {
    ...
    const binders = clauses.flatMap(
        c => findActiveTypeBindersInBlock(c, symbol, startNode) || []
    );
    // TODO: Could this be more accurate?
    // It is more complicated than if-statements, because case clauses might fall-through.
    binders.push(
        ...findActiveTypeBindersOutOfConditional(node, symbol, startNode)
    );
...
function findActiveTypeBindersInForStatement(node, symbol, startNode) {
    ...
    const statementBinders =
        Ast.findActiveTypeBindersInStatement(statement, symbol, startNode) ||
        [];
    const outerBinders = findActiveTypeBindersOutOfForStatement(
        node,
        symbol,
        startNode
    );
...
function findActiveTypeBindersOutOfForStatement(node, symbol, startNode) {
    ...
    if (
        condition &&
        (conditionBinders = Ast.findActiveTypeBindersInLeftSibling(
            condition,
            symbol,
            startNode,
            node
        ...
        initializer &&
        (initializerBinders = Ast.findActiveTypeBindersInLeftSibling(
            initializer,
            symbol,
            startNode,
            node
...
function findActiveTypeBindersInDoStatement(node, symbol, startNode) {
    ...
    const binders =
        ...
        [];
    binders.push(
        ...findActiveTypeBindersOutOfDoStatement(node, symbol, startNode)
    );
    return binders;
}
...
function findStopNodeOutOfConditional(node, startNode) {
    const parentConditionalStatement = Ast.findAncestor(
        node.parent,
        conditionalNodes
    );
    if (
        parentConditionalStatement &&
        !Ast.isInner(parentConditionalStatement, startNode) &&
        !isFunctionBetween(node, parentConditionalStatement)
    ) {
...
function findActiveTypeBindersInCallExpression(
    node,
    symbol,
    startNode,
    stopNode
    ...
    return (
        ...
        (ts.isClassLike(node.callee.parent) &&
            findActiveTypeBindersInMembers(node.callee.parent, symbol)) ||
        Ast.findActiveTypeBinders(node, symbol, startNode, stopNode)
    );
}
...
function findActiveTypeBindersInCallSite(node, symbol, startNode, stopNode) {
    ...
    return (
        Ast.findActiveTypeBinders(node, symbol, startNode, localStopNode) ||
 ...
 */
function findActiveTypeBindersInAssignmentLike(
    node,
    symbol,
    startNode,
    stopNode
    ...
    switch (node.kind) {
        ...
        case ts.SyntaxKind.BinaryExpression:
            return (
                ...
                node.right &&
                Ast.isInner(node.right, checkNode)
            );
    }
};
...
Ast.isDeclaredInFunction = (node, symbol, functionNode) => {
    ...
    const leftSibling = Ast.findLeftSibling(node);
    return Ast.isDeclaredInFunction(
 ...
 * @param {ts.Node} node
 */
Ast.findLastStatement = node => Ast.findLastNodeOfArray(node, 'statements');

/**
 * @param {ts.Node} node
 */
Ast.findLastParameter = node => Ast.findLastNodeOfArray(node, 'parameters');
...
Ast.findStatementAncestor = node => {
    let currentNode = node;
    while (currentNode) {
        if (Ast.isStatement(currentNode)) {
            return currentNode;
        }
        currentNode = currentNode.parent;
    }
 ...
 */
Ast.isParameterName = node =>
    node.parent.kind === ts.SyntaxKind.Parameter && node.parent.name === node;
 ...
 */
Ast.isGetterName = node =>
    node.parent.kind === ts.SyntaxKind.GetAccessor && node.parent.name === node;

/**
 * @param {ts.Identifier} node
 */
Ast.isSetterName = node =>
    node.parent.kind === ts.SyntaxKind.SetAccessor && node.parent.name === node;
...
Ast.isClassMember = node => node.parent && ts.isClassLike(node.parent);
