...
SignatureHelp.onSignatureHelp = info => {
    ...
    const offset =
        ast.getPositionOfLineAndCharacter(position.line, position.character) -
        1;
    ...
    const activeSignature = context.activeSignatureHelp
        ? context.activeSignatureHelp.activeSignature
        : 0;
    if (callees.length > 1) {
        setMetadata(ast, call, callees[activeSignature]);
    }
    const signatures = callees.map(callee =>
        computeFunctionSignature(callee, call)
    );
...
const computeParametersSignature = callee => {
    ...
    const computeParameterSignature = p => {
        ...
        const symbol = Ast.lookUp(Ast.findLastParameter(callee), parameterName);

        const inducedBinders =
            symbol && symbol.binders.filter(b => b.carrier.induced);
...
const computeFunctionSignature = (callee, call) => {
    ...
    return {
        ...
        label: signature,
        parameters: parametersSignature.map(p =>
            vscodeLanguageServer.ParameterInformation.create(
                p /* parameter documentation */
            )
        ),
    };
};
...
function computeDocumentation(node) {
    ...
    return closestStatement.jsDoc
        ? computeJSDocDocumentation(closestStatement)
        : '';
}
...
const computeActiveParameter = (call, offset) => {
    ...
    const leftParenthesisToken = callChildren.find(
        c => c.kind === ts.SyntaxKind.OpenParenToken
    );
