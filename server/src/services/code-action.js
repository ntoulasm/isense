...
function computeConstantAssignmentCodeAction(ast, diagnostic) {
    const offset = ast.getPositionOfLineAndCharacter(diagnostic.range.start.line, diagnostic.range.start.character);
    /**
     * @type {ts.Node}
     */
    const node = Ast.findInnerMostNode(ast, offset, n => n.kind === ts.SyntaxKind.BinaryExpression);
    /**
     * @type {ts.Node}
     */
    ...
    const declarationStart = declarationSource.getLineAndCharacterOfPosition(declarationList.parent.getStart());
    const declarationEnd = declarationSource.getLineAndCharacterOfPosition(declarationList.parent.end);
     ...
     */
    const fix = vscodeLanguageServer.CodeAction.create(`Change const to let declaration`, vscodeLanguageServer.CodeActionKind.QuickFix);
    fix.diagnostics = [];
    ...
    if(declarationList.declarations.length === 1) {
        fix.edit.changes[actionUri] = [
            ...
            vscodeLanguageServer.TextEdit.insert(declarationStart, `let ${declaration.getText()};`)
        ];
    } else if(declarationList.declarations.indexOf(declaration) === 0) { // const x = 2, ...;
        const deletionStartNode = declaration;
        const deletionEndNode = getNextCommaToken(declarationList, declaration) || declaration
        const deletionStart = declarationSource.getLineAndCharacterOfPosition(deletionStartNode.getFullStart());
        const deletionEnd = declarationSource.getLineAndCharacterOfPosition(deletionEndNode.end);
        const deletionRange = vscodeLanguageServer.Range.create(deletionStart, deletionEnd);
        const insertionStart = declarationSource.getLineAndCharacterOfPosition(declarationList.parent.end);
        fix.edit.changes[actionUri] = [
            vscodeLanguageServer.TextEdit.del(deletionRange),
            vscodeLanguageServer.TextEdit.insert(insertionStart, `\n${computeLeadingWhitespaces(declarationList.parent)}let ${declaration.getText()};`)
        ];
    } else {
        const deletionStartNode = getPreviousCommaToken(declarationList, declaration);
...
function getNextCommaToken(declarationList, declaration) {
    ...
    const declarationIndex = children.indexOf(declaration);
    console.assert(declarationIndex !== -1);
    if(declarationIndex === children.length - 1) { return ; }
