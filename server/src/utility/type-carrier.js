...
} = require('../analyzer/implicit-return-detector');
...
TypeCarrier.evaluate = carrier => {
    ...
    if (f) {
        ...
        console.assert(
            false,
            `Trying to evaluate unknown carrier '${carrier.kind}'`
        );
    }
};
...
evaluateFunctions[TypeCarrier.Kind.Variable] = carrier =>
    Ast.findActiveTypeBinders(carrier.node, carrier.symbol).flatMap(b =>
        TypeCarrier.evaluate(b.carrier)
    );

const evaluateBinaryExpressionFunctions =
    (TypeCarrier.evaluateBinaryExpressionFunctions = []);
...
evaluateFunctions[TypeCarrier.Kind.BinaryExpression] = carrier => {
    ...
    if (f) {
        ...
        console.assert(
            false,
            `Trying to evaluate unknown binary expression '${carrier.op.kind}'`
        );
    }
};

const evalutePrefixUnaryExpressionFunctions =
    (TypeCarrier.evaluatePrefixUnaryExpressionFunctions = []);
...
evaluateFunctions[TypeCarrier.Kind.PrefixUnaryExpression] = carrier => {
    ...
    if (f) {
        ...
        console.assert(
            false,
            `Trying to evaluate unknown prefix binary expression '${carrier.op}'`
        );
    }
};
...
TypeCarrier.computePlausibleReturnTypes = func => {
    ...
    const plausibleTypes = func.returnTypeCarriers.flatMap(c =>
        TypeCarrier.evaluate(c)
    );
...
evaluateBinaryExpressionFunctions[ts.SyntaxKind.PlusToken] = carrier => {
    ...
    const anyPlus = other => {
        ...
        return [TypeInfo.createString(), TypeInfo.createNumber()];
    };
    ...
    const plus = (left, right) => {
        if (left.type === TypeInfo.Type.Any) {
            ...
            TypeInfo.isStringLike(left) ||
            TypeInfo.isStringLike(right)
        ) {
            if (left.hasValue && right.hasValue) {
                const leftString = TypeInfo.toString(left);
                const rightString = TypeInfo.toString(right);
                return [
                    TypeInfo.createString(leftString.value + rightString.value),
                ];
            }
            return [TypeInfo.createString()];
        } else {
            if (left.hasValue && right.hasValue) {
                ...
                return [
                    TypeInfo.createNumber(leftNumber.value + rightNumber.value),
                ];
            }
...
evaluateBinaryExpressionFunctions[ts.SyntaxKind.MinusToken] =
    ...
    evaluateBinaryExpressionFunctions[
                    ...
                    if (l.hasValue && r.hasValue) {
                        ...
                        info.push(
                            TypeInfo.createNumber(
                                eval(leftNumber.value + op + rightNumber.value)
                            )
                        );
                    } else {
 ...
 * @returns {Array<isense.TypeInfo>}
 */
evaluateBinaryExpressionFunctions[ts.SyntaxKind.EqualsEqualsToken] =
    carrier => {
        const leftInfo = TypeCarrier.evaluate(carrier.left);
        const rightInfo = TypeCarrier.evaluate(carrier.right);
 ...
 * @returns {Array<isense.TypeInfo>}
 */
evaluateBinaryExpressionFunctions[ts.SyntaxKind.ExclamationEqualsToken] =
    carrier => {
        const booleanInfo =
            evaluateBinaryExpressionFunctions[ts.SyntaxKind.EqualsEqualsToken](
                carrier
 ...
 * @returns {Array<isense.TypeInfo>}
 */
evaluateBinaryExpressionFunctions[ts.SyntaxKind.EqualsEqualsEqualsToken] =
    carrier => {
        const leftInfo = TypeCarrier.evaluate(carrier.left);
        const rightInfo = TypeCarrier.evaluate(carrier.right);
 ...
 * @returns {Array<isense.TypeInfo>}
 */
evaluateBinaryExpressionFunctions[ts.SyntaxKind.ExclamationEqualsEqualsToken] =
    carrier => {
        const booleanInfo =
            TypeCarrier.evaluateBinaryExpressionFunctions[
 ...
 * @returns {Array<isense.TypeInfo>}
 */
evaluateBinaryExpressionFunctions[ts.SyntaxKind.AmpersandAmpersandToken] =
    carrier => {
        const leftInfo = TypeCarrier.evaluate(carrier.left);
        const rightInfo = TypeCarrier.evaluate(carrier.right);
        const info = [];
 ...
 * @returns {Array<isense.TypeInfo>}
 */
evalutePrefixUnaryExpressionFunctions[ts.SyntaxKind.PlusPlusToken] =
    carrier => {
        return TypeCarrier.evaluate(carrier.operand).map(o => {
            const number = TypeInfo.toNumber(o);
            if (number.hasValue) {
 ...
 * @returns {Array<isense.TypeInfo>}
 */
evalutePrefixUnaryExpressionFunctions[ts.SyntaxKind.MinusMinusToken] =
    carrier => {
        return TypeCarrier.evaluate(carrier.operand).map(o => {
            const number = TypeInfo.toNumber(o);
            if (number.hasValue) {
 ...
 * @returns {Array<isense.TypeInfo>}
 */
evalutePrefixUnaryExpressionFunctions[ts.SyntaxKind.ExclamationToken] =
    carrier => {
        return TypeCarrier.evaluate(carrier.operand).map(o => {
...
TypeCarrier.removeDuplicates = typeInfo => {
    ...
    for (const t of typeInfo) {
        if (
            t.hasValue ||
            !info.find(nt => !nt.hasValue && nt.type === t.type)
        ) {
            info.push(t);
        }
    }
