...
Signature.compute = (
	...
	const typeInfoToText = t =>
		computeValues && t.hasValue
	...
	return (
		...
		symbol.name +
		': ' +
		typeInfo.map(typeInfoToText).join(typeSeparator)
	);
};
...
function stringifyObject(node, typeInfo) {
	if (seen.has(typeInfo)) {
		return '[Circural Reference]';
	} else {
		seen.add(typeInfo);
	}

	if (typeInfo.properties.getSymbols().length === 0) {
		return '';
	}
	...
	for (const [, property] of Object.entries(
		typeInfo.properties.getSymbols()
	)) {
